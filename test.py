...
model = CascadeBoth(num_out_classes=3, input_channels=4, init_coarse_channels=4, init_fine_channels=16, testing=True)
model.to(device)

# summary(model, input_size=(4, 160, 160, 128), device='cuda')

model.load_state_dict(torch.load("/home/kemove/lt/2stage/weights/best07091334.pth"))
model.eval()
...
with torch.no_grad():
    with tqdm(test_loader, desc="Testing", unit="batch") as tqdm_lodaer:
        for sample_idx, test_data in enumerate(tqdm_lodaer):
            ...
            # outputs = [post_trans(i) for i in decollate_batch(output)]
