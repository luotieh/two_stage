...
model_coarse.load_state_dict(torch.load("/home/kemove/lt/2stage/weights/endcoarse07012337.pth"))
model_fine.load_state_dict(torch.load("/home/kemove/lt/2stage/weights/endfine07012337.pth"))
model_coarse.eval()
model_fine.eval()
...
with torch.no_grad():
    with tqdm(test_loader, desc="Testing", unit="batch") as tqdm_lodaer:
        for sample_idx, test_data in enumerate(tqdm_lodaer):
            ...
            coarse_outputs = inference(inputs, model=model_coarse)
            ...
            output = torch.where(restore_output > 0.5, 1, 0)
